...
@section('content')
    {{-- Hero Section untuk Kategori Galeri --}}
    <div class="relative h-72 md:h-96 lozad page-banner"
         data-background-image="{{ asset('images/galeri-kategori-banner.jpg') }}">
        <div class="hero-overlay-darker"></div>
        <div class="hero-content-container container mx-auto px-4 h-full flex items-center justify-center text-center">
            <h1 class="text-3xl md:text-5xl font-extrabold text-white relative z-10 leading-tight text-shadow-strong">
                Galeri Kategori: <br class="md:hidden">{{ $kategori->nama_kategori }}
            </h1>
        </div>
    </div>
    ...
    <div class="container mx-auto px-4 mt-8 mb-6">
        <nav aria-label="Breadcrumb">
            <ol class="flex items-center space-x-2 text-sm text-gray-500">
                ...
                <li><a href="{{ route('galeri.public') }}" class="hover:text-blue-600 hover:underline">Galeri</a></li>
                <li><svg class="w-4 h-4 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg></li>
    ...
    <div class="container mx-auto px-4 py-8">
        @if($galeris->isEmpty())
            <p class="text-center text-gray-600 text-lg">Belum ada galeri dalam kategori ini.</p>
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach($galeris as $galeri)
                    <a href="{{ route('galeri.show.public', ['slug' => $galeri->slug]) }}" class="gallery-card block">
                        <div class="relative overflow-hidden">
                            @if($galeri->gambar)
                                <img src="{{ asset('storage/' . $galeri->gambar) }}" alt="{{ $galeri->judul }}" class="lozad" data-src="{{ asset('storage/' . $galeri->gambar) }}">
                            @else
                                <img src="https://placehold.co/600x400/cccccc/333333?text=No+Image" alt="No Image" class="lozad">
                            @endif
                            <div class="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent"></div>
                            <span class="absolute top-4 left-4 gallery-meta category-badge">
                                {{ $galeri->kategoriGaleri->nama_kategori ?? 'Uncategorized' }}
                            </span>
                        </div>
                        <div class="p-6">
                            ...
                            <div class="flex items-center text-sm text-gray-500 mb-4">
                                <span class="gallery-meta">
                                    ...
                                    {{-- PERBAIKAN FORMAT TANGGAL YANG AMAN --}}
                                    {{ $galeri->created_at->format('d F Y') }}
                                </span>
                                <span class="gallery-meta ml-4">
                            ...
                            <p class="text-gray-700 text-base mb-4 line-clamp-3">
                                {{ Str::limit(strip_tags($galeri->keterangan), 150) }}
                            </p>
